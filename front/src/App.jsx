...
function App(){
    ...
    const { p1, p2 } = useContext(UserContext)
    ...
    function CheckWin(shape , board){
        ...
        if (didWin){
            if (shape == 'cross') {
                setWins(p1.name)
                setResultP1({name: p1.name , status:'WIN'})
                setResultP2({name: p2.name , status:'LOSE'})

            }else{
                setWins(p2.name)
                setResultP1({name: p1.name , status:'LOSE'})
                setResultP2({name: p2.name , status:'WIN'})
            }
    ...
    return <div className="bodyy" >
        <Context.Provider value={[done , setDone] }>
            <Names p1={p1.name} p2={p2.name} wins={wins} />
